...
const ActivityItem = ({ card, addJoin, DropJoin, user, deleteCard }) => {
  ...
  const [showDeletedit, setShowDeletedit] = useState(false);
  ...
  useEffect(() => {
    if ((user && user.uid) === userUid) {
      setShowJoin(false);
      setShowDeletedit(true);
    } else {
      setShowDropOut(false);
      setShowJoin(true);
      setJoinListShow(false);
      setShowDeletedit(false);
      if (
        user &&
        joinList &&
  ...
  const clickEdit = (card) => {
    navigate({ pathname: "/update" }, { state: card });
  };
      ...
      <div className={styles.card}>
        ...
        <div className={styles.btnBox}>
          <div className={styles.display}>
            ...
            <div
              className={`${styles.Deletedit} ${styles.deletedit} ${
                showDeletedit && styles.showDeletedit
              }`}
            >
              <button
              ...
              <button
                onClick={() => clickEdit(card)}
                className={`${styles.btns} ${styles.edit}`}
              >
                Edit
              </button>
            </div>
